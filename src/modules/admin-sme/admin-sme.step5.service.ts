 ...
 * Handles CR1, CR2, CR8, CR12, certificate_of_incorporation, etc.
 */
export abstract class AdminSMEStep5Service {
  /**
   * Save Company Info Documents
   * Upserts business documents for company info (admin override - no clerkId required)
   */
  static async saveCompanyInfoDocuments(
    ...
    try {
      ...
      const normalized = docsArray.map((d) => ({
        ...
        docYear: null, // Company info docs don't typically have years
        docBankName: null, // Company info docs don't have bank names
      }));
      ...
      logger.info("[AdminSME Step5] Step 5 saved", {
        userId,
      ...
      return {
        ...
        currentStep: progressResult?.currentStep ?? 5,
        completedSteps: (progressResult?.completedSteps as number[]) ?? [5],
        user: {
      ...
      logger.error("[AdminSME Step5] Error saving Step 5", {
        error: error?.message,
        userId,
      });
      if (error?.status) throw error;
      throw httpError(500, "[STEP5_ERROR] Failed to save company info documents");
    }
  }
}
