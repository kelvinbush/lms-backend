  ...
  static async getDocumentsForVerification(
    ...
    try {
      ...
      for (const verification of verifications) {
        verificationMap.set(
          `${verification.documentType}:${verification.documentId}`,
          verification
        );
      }
      ...
      const personalDocumentsWithStatus: KycKybVerificationModel.DocumentItem[] = personalDocs.map(
        (doc) => {
      ...
      const businessDocumentsWithStatus: KycKybVerificationModel.DocumentItem[] = businessDocs.map(
        (doc) => {
